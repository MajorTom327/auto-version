...
const isCloned = (workDir) => fs.existsSync(path.resolve(workDir, '.git'));
...
const getAccessToken = (required) => getInput('GITHUB_TOKEN', { required });
const getOctokit = (token) => getOctokit$1(token ?? getAccessToken(true), {});
...
const useNpm = (workDir, pkgManager = '') => 'npm' === pkgManager ||
    ('yarn' !== pkgManager && (fs.existsSync(path.resolve(workDir, 'package-lock.json')) ||
...
const isCommandDebug = () => getInput('UTILS_COMMAND_DEBUG') === 'true' || process.env.UTILS_COMMAND_DEBUG === 'true';
const isOutputDebug = () => getInput('UTILS_OUTPUT_DEBUG') === 'true' || process.env.UTILS_OUTPUT_DEBUG === 'true';
...
export { ApiHelper, Command, contextHelper as ContextHelper, GitHelper, utils as Utils };
